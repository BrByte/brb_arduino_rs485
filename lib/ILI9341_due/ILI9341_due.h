...
#define _textScale 1
  ...
  public:
	...
	inline __attribute__((always_inline)) void fillArc(uint16_t x, uint16_t y, uint16_t radius, uint16_t thickness, float start, float end, uint16_t color)
	{
		beginTransaction();
  ...
  protected:
	...
	__attribute__((always_inline)) void spiwrite(uint8_t c)
	{
#if defined ARDUINO_ARCH_AVR
		SPDR = c;
		asm volatile("nop");
	...
	__attribute__((always_inline)) void spiwrite16(uint16_t d)
	{
#if defined ARDUINO_ARCH_AVR
		SPDR = highByte(d);
		while (!(SPSR & _BV(SPIF)))
	...
	__attribute__((always_inline)) void write8_cont(uint8_t c)
	{
#if SPI_MODE_NORMAL
		spiwrite(c);
#elif SPI_MODE_EXTENDED
	...
	// CS and DC have to be set prior to calling this method
	inline __attribute__((always_inline)) void write8_last(uint8_t c)
	{
#if SPI_MODE_NORMAL
		spiwrite(c);
		disableCS();
	...
	__attribute__((always_inline)) void write16_cont(uint16_t d)
	{
#if SPI_MODE_NORMAL
		spiwrite16(d);
#elif SPI_MODE_EXTENDED
	...
	__attribute__((always_inline)) void write16_last(uint16_t d)
	{
#if SPI_MODE_NORMAL
		spiwrite16(d);
		disableCS();
...
#endif
	void
	setAddrAndRW_cont(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
	{
		setDCForCommand();
		write8_cont(ILI9341_CASET); // Column addr set
		setDCForData();
	...
	inline __attribute__((always_inline)) void setAddrAndRW_cont_inline(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
	{
		setDCForCommand();
		write8_cont(ILI9341_CASET); // Column addr set
		setDCForData();
...
#endif
	void
	setColumnAddr(uint16_t x, uint16_t w)
	{
		setDCForCommand();
		write8_cont(ILI9341_CASET); // Column addr set
		setDCForData();
...
#endif
	void
	setRowAddr(uint16_t y, uint16_t h)
	{
		setDCForCommand();
		write8_cont(ILI9341_PASET); // Row addr set
		setDCForData();
	...
	inline __attribute__((always_inline)) void setRW() // RAM Write
	{
		setDCForCommand();
		write8_cont(ILI9341_RAMWR); // RAM write
	}
	...
	// Enables CS, sets DC for Command, writes 1 byte
	// Does not disable CS
	inline __attribute__((always_inline)) void writecommand_cont(uint8_t c)
	{
		setDCForCommand();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	inline __attribute__((always_inline)) void writecommand_last(uint8_t c)
	{
		setDCForCommand();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	// Does not disable CS
	inline __attribute__((always_inline)) void writedata8_cont(uint8_t c)
	{
		setDCForData();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	__attribute__((always_inline)) void writedata8_last(uint8_t c)
	{
		setDCForData();
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		enableCS();
	...
	inline __attribute__((always_inline)) void write_cont(uint16_t *buf, uint32_t n)
	{
#if SPI_MODE_NORMAL
		spiTransfer(buf, n);
#elif SPI_MODE_EXTENDED
	...
	inline __attribute__((always_inline)) void write_cont(const uint16_t *buf, uint32_t n)
	{
#if SPI_MODE_NORMAL
		spiTransfer(buf, n);
#elif SPI_MODE_EXTENDED
	...
	inline __attribute__((always_inline)) void read_cont(uint8_t *buf, uint32_t n)
	{
#if SPI_MODE_DMA
		dmaReceive(buf, n);
#endif
	...
	inline __attribute__((always_inline)) void writeScanline16(uint32_t n)
	{
		/*setDCForData();
		enableCS();*/
#if SPI_MODE_NORMAL
	...
	inline __attribute__((always_inline)) void writeScanlineLooped(uint32_t n)
	{
	...
	__attribute__((always_inline)) void fillScanline16(uint16_t color)
	{
	...
	__attribute__((always_inline)) void fillScanline16(uint16_t color, uint16_t len)
	{
	...
	__attribute__((always_inline)) void writeHLine_cont_noCS_noFill(int16_t x, int16_t y, int16_t w)
	{
#ifdef ARDUINO_ARCH_AVR
	...
	__attribute__((always_inline)) void writeHLine_cont_noCS_noScanline(int16_t x, int16_t y, int16_t w, uint16_t color)
	{
		setAddrAndRW_cont(x, y, w, 1);
		setDCForData();
		while (w-- > 0)
	...
	// the scanlineBuffer not filled on every call
	__attribute__((always_inline)) void writeVLine_cont_noCS_noFill(int16_t x, int16_t y, int16_t h)
	{
#ifdef ARDUINO_ARCH_AVR
	...
	__attribute__((always_inline)) void writeVLine_cont_noCS_noScanline(int16_t x, int16_t y, int16_t h, uint16_t color)
	{
		setAddrAndRW_cont(x, y, 1, h);
		setDCForData();
		while (h-- > 0)
	...
	inline __attribute__((always_inline)) void writePixel_cont(int16_t x, int16_t y, uint16_t color)
	{
	...
	inline __attribute__((always_inline)) void writePixel_last(int16_t x, int16_t y, uint16_t color)
	{
...
#endif
	void
	drawPixel_cont(int16_t x, int16_t y, uint16_t color)
	{
	...
	inline __attribute__((always_inline)) void drawPixel_last(int16_t x, int16_t y, uint16_t color)
	{
	...
	// Enables CS
	inline __attribute__((always_inline)) void enableCS()
	{
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		*_csport &= ~_cspinmask;
#endif
	...
	// Disables CS
	inline __attribute__((always_inline)) void disableCS()
	{
#if SPI_MODE_NORMAL | SPI_MODE_DMA
		*_csport |= _cspinmask;
	...
	// Sets DC to Data (1)
	inline __attribute__((always_inline)) void setDCForData()
	{
		*_dcport |= _dcpinmask;
	...
	inline __attribute__((always_inline)) void setDCForCommand()
	{
		*_dcport &= ~_dcpinmask;
	}
#ifdef ARDUINO_ARCH_AVR
	inline __attribute__((always_inline)) void spiTransfer(const uint8_t *buf, uint32_t count)
	{
		SPDR = *buf;
		while (--count > 0)
		{
	...
	inline __attribute__((always_inline)) void spiTransfer(uint16_t *buf, uint32_t count)
	{
		uint16_t pixel = *buf;
		SPDR = highByte(pixel);
		while (!(SPSR & _BV(SPIF)))
...
#define ILI_SPI_RX_IDX 2
	...
	//------------------------------------------------------------------------------
	/** SPI send a byte */
	__attribute__((always_inline)) void dmaSend(uint8_t b)
	{
		dmaSpiTransfer(b);
	}

	__attribute__((always_inline)) void dmaSend(uint16_t w)
	{
		dmaSpiTransfer(w);
	}
	//------------------------------------------------------------------------------
